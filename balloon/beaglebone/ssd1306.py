...
class SSD1306:
    ...
    def start_scroll_up(self, start=0, stop=32):
        self.command(self.SET_VERT_SCROLL_AREA, 0x00, self.rows)
        self.command(self.VERT_AND_RIGHT_HORIZ_SCROLL,
                     0x00, start, 0x00, stop, 0x01)
        self.command(self.ACTIVATE_SCROLL)

    def start_scroll(self, start=0, stop=15, direction=1):
        direction_cmd = self.RIGHT_HORIZ_SCROLL
        if direction < 0:
            direction_cmd = self.LEFT_HORIZ_SCROLL

        self.command(direction_cmd, 0x00, start, 0x00, stop, 0x01, 0xFF)
        self.command(self.ACTIVATE_SCROLL)
    ...
    def draw_text2(self, x, y, string, size=2, space=1, invert=False):
        font_bytes = self.font.bytes
        font_rows = self.font.rows
        font_cols = self.font.cols

        if invert:
            width = len(string) * font_cols
            width *= size
            width += (len(string) - 1) * space
            for c in string:
                width += space
            for ix in range(x, x + width):
                for iy in range(y, y + font_rows):
                    self.draw_pixel(ix, iy, 1)
